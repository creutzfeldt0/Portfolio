...
public class UICenterOnChild : MonoBehaviour
{
            ...
            if (mCenteredObject != null && mCenteredObject.transform == (list != null ? list[index] : trans.GetChild(index)))
            {
                ...
                if (Mathf.Abs(delta) > nextPageThreshold)
                {
                    if (delta > nextPageThreshold)
                    {
                        ...
                        {
                            // Next page
                            closest = GetNextPage(trans, list, ignoredIndex);
                        }
                        else
                        {
                            // Previous page
                            closest = GetPrevPage(trans, list, ignoredIndex);
                        }
                    }
    ...
    /// <summary>
    /// 이전 페이지 오브젝트 불러오기 위한 함수 정의
    /// </summary>
    /// <param name="trans"></param>
    /// <param name="list"></param>
    /// <param name="ignoredIndex"></param>
    /// <returns></returns>
    private Transform GetPrevPage(Transform trans, List<Transform> list, int ignoredIndex)
    {
        Transform closest = null;
    ...
    /// <summary>
    /// 다음 페이지 오브젝트 불러오기 위한 함수 정의
    /// </summary>
    /// <param name="trans"></param>
    /// <param name="list"></param>
    /// <param name="ignoredIndex"></param>
    /// <returns></returns>
    private Transform GetNextPage(Transform trans, List<Transform> list, int ignoredIndex)
    {
        Transform closest = null;
    ...
    {
        UIWidget.Pivot pivot = mScrollView.contentPivot;
    ...
    bool CheckChildeRange(Transform _transform, int _nIndex)
    {
        return _nIndex >= 0 && _nIndex < _transform.childCount;
    }
