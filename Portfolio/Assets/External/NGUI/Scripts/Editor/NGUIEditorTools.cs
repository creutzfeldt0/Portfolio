...
static public class NGUIEditorTools
{
	...
	static public GameObject SelectedRoot (bool createIfMissing)
	{
		...
		if (createIfMissing && go == null)
		{
			...
			if (go == null)
			{
				UIPanel panel = Object.FindAnyObjectByType<UIPanel>();
	...
	static public int GetClassID (System.Type type)
	{
		...
		var entityId = ob.FindProperty("m_Script").objectReferenceEntityIdValue;
    	int classID = entityId.GetHashCode();
	...
	static public SerializedObject ReplaceClass (MonoBehaviour mb, System.Type type)
	{
		...
		EntityId entityId = EntityId.FromULong((ulong)id);
		ob.FindProperty("m_Script").objectReferenceEntityIdValue = entityId;
	...
	static public SerializedObject ReplaceClass (MonoBehaviour mb, int classID)
	{
		var ob = new SerializedObject(mb);
		ob.Update();
		EntityId entityId = EntityId.FromULong((ulong)classID);
		ob.FindProperty("m_Script").objectReferenceEntityIdValue = entityId;
		ob.ApplyModifiedProperties();
		ob.Update();
		return ob;
	...
	{
		EntityId entityId = EntityId.FromULong((ulong)GetClassID(type));
		ob.FindProperty("m_Script").objectReferenceEntityIdValue = entityId;
		ob.ApplyModifiedProperties();
		ob.Update();
	}
	...
	static public Object GUIDToObject(string guid)
	{
		...
		// 1. GUID를 실제 에셋 파일 경로로 변환합니다.
		string path = AssetDatabase.GUIDToAssetPath(guid);
		if (string.IsNullOrEmpty(path)) return null;

		// 2. 최신 API를 이용하여 해당 경로의 메인 에셋을 깔끔하게 로드하여 반환합니다.
		return AssetDatabase.LoadMainAssetAtPath(path);
	}
