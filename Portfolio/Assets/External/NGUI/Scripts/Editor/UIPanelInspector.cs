...
public class UIPanelInspector : UIRectEditor
{
    ...
    private void ClearFooComponents()
    {
        ...
        {
            Component[] fooComponents = obj.GetComponents<Rigidbody>();
            foreach (Component currentComponent in fooComponents)
            {
                DestroyImmediate(currentComponent, true);
            }
        }
        ...
        //var oldName = importer.assetBundleName;
        //var newName = string.Format("{0}/{1}", pathname, string.IsNullOrEmpty(bundleName) ? Path.GetFileNameWithoutExtension(path).Replace(pathname, string.Empty) : bundleName);

        //Log.Warning("SetAssetBundleNames - Target: {0}, Old: {1}, New: {2}", path, string.IsNullOrEmpty(oldName) ? "None" : oldName, newName);
