...
public class UIInput : MonoBehaviour
{
    ...
    //Custume by. HJ (자식과 뎁스 교환을 위한 상태 변수)
    public bool childeDepthChange = false;
    ...
    //나와 부모의 뎁스를 교체하기 위한 함수 정의
    private void ChangeDepth(UITexture uITexture)
    {
        if (childeDepthChange)
        {
            UILabel uILabel = uITexture.parent.GetComponent<UILabel>();
            if (uILabel != null)
            {
                //뎁스 교체
                int iTemp = uITexture.depth;
                uITexture.depth = uILabel.depth;
                uILabel.depth = iTemp;
            }
        }
    }
