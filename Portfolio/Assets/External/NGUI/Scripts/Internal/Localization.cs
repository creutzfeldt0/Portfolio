...
static public class Localization
{
...
#if true
	//[2023.02.01][rex] 이제는 Text/TextCommon.csv 것을 읽어 mDictionary 를 채운다.
	//	- 헌데, TextCommon에 없는 언어를 누른 후 있는 언어를 누르면,
	//	- SelectLanguage()에서 실패 후 리턴 못하고 Clear() 처리된다. <- 오류,
	//	- 원래 의도는 그 이후 {textDirectory}/text/{언어}.csv 를 읽게 하려는 것 같은데,
	//	- 그렇다면 LoadDictionary() 위치가 한발 늦음, 이후 계속 비정상 작동.
	//	- 순서를 조정키로 한다.
	static bool LoadAndSelect(string value)
	{
		...
		//SelectLanguage:false는 TextCommon에 없는 언어라는 것.
		//원래 의도인 {언어}.csv 방식은 사용 중이 아니니 그냥 리턴.
		return false;
	}
#else
	static bool LoadAndSelect (string value)
	...
	{
		//mLanguageIndex = -1; //[2023.02.01][rex] 언어가 없다면 기존 것 사용하는 게 더 안전.
	...
	/*private static void BroadcastLocalize()
	{
		UILabelEx[] labelExes;
		UIBroadcastReceiver_Text[] receiver_Texts;

		for (int i = 0; i < UIRoot.list.Count; i++)
		{
			labelExes = UIRoot.list[i].GetComponentsInChildren<UILabelEx>( true );
			receiver_Texts = UIRoot.list[i].GetComponentsInChildren<UIBroadcastReceiver_Text>( true );

			//LabelEX 언어변환..
			for (int j = 0; j < labelExes.Length; j++)
			{
				labelExes[j].OnLocalize();
			}

			//Text를 코드에서 넣고 있는 곳 변환(상속 관계가 좀 더 잘되있었더라면 필요없었을듯)..
			for (int j = 0; j < receiver_Texts.Length; j++)
			{
				receiver_Texts[j].RefreshText();
			}
		}
	}*/
